...
function toFormValues(boxer?: Boxer | null): BoxerFormValues {
  return {
    ...
    weightGoal: boxer?.weightGoalKg != null ? String(boxer.weightGoalKg) : '',
...
export function BoxerForm({
  ...
  function validate(): boolean {
    ...
    if (values.weightGoal && (isNaN(Number(values.weightGoal)) || Number(values.weightGoal) <= 0)) {
      errs.weightGoal = "L'objectif doit être un nombre positif.";
    }
    setErrors(errs);
  ...
  function handleSubmit(e: FormEvent) {
    ...
    onSubmit({
      ...
      weightGoalKg: values.weightGoal ? Number(values.weightGoal) : null,
  ...
  return (
    <form onSubmit={handleSubmit} className="px-4 py-4 space-y-5">
      ...
      <div>
        <label className={labelClass}>Objectif de poids (kg)</label>
        <input
          className={inputClass}
          value={values.weightGoal}
          onChange={(e) => set('weightGoal', e.target.value)}
          placeholder="Optionnel, ex : 69"
          inputMode="decimal"
        />
        {errors.weightGoal && <p className="text-red-500 text-xs mt-1">{errors.weightGoal}</p>}
      </div>
